...
///
/// Honors event restrictions: a token operators have marked for drop
/// (load-shedding) stays fully silent, so invalid payloads from an abusive
/// token can't force a warning produce the valid path would have shed.
