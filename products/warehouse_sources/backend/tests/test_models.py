...
@pytest.mark.parametrize(
    "s3_folder_path,sync_type_config,expected",
    [
...
def test_resolved_s3_folder_path(
    s3_folder_path: str | None, sync_type_config: dict | None, expected: str | None
) -> None:
    ...
    schema = ExternalDataSchema(s3_folder_path=s3_folder_path, sync_type_config=sync_type_config)
    assert schema.resolved_s3_folder_path == expected
